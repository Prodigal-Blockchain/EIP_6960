    ...
    ERC721URIStorage
{
    ...
    function setTokenURI(
        ...
        string memory _assetURI = MetaDataDescriptor.generateJSON(
            fullName,
            assetURI
        );
    ...
    function addEmployee(
        ...
        require(
            msg.sender == departments[movieId][departmentId].departmentManager,
            "Department:not department manager"
        );
        uint256 EMPLOYEE_INDEX = EMPLOYEE_COUNTER[movieId][departmentId];
        employees[movieId][departmentId][EMPLOYEE_INDEX] = Employee(
            employeeAddress,
            movieId,
            departmentId,
            EMPLOYEE_INDEX,
        ...
        mappingToArrays.addToTwoKeyMapping(
            movieId,
            departmentId,
            EMPLOYEE_INDEX
        );
    ...
    function PayMaintainance(
        ...
        require(movieExists[movieId][msg.sender], "Movie: Doesn't exists");
        require(
            _balances[movieId][msg.sender][0] != 0,
            "Movie:Insufficient balance"
        );
        require(
    ...
    function transferEmployeeBalance(
        ...
        require(
            employeeExists[movieId][departmentId][fromEmployeeId],
            "Employee:doesn't exists"
        );
        require(
            employeeExists[movieId][departmentId][toEmployeeId],
            "Employee:doesn't exists"
        );
        _balances[movieId][from][fromEmployeeId] -= transferValue;
        _balances[movieId][to][toEmployeeId] += transferValue;
        emit DepartmentFundTransferred(from, to, fromEmployeeId, toEmployeeId);
    }
    ...
    function addEmployeesalary(uint256 movieId, uint256 departmentId) public {
        ...
        require(
            !departments[movieId][departmentId].salaryPaid,
            "Department: salary is paid"
        );

        uint256 totalEmployees = departments[movieId][departmentId]
            .noOfEmployees;
        uint256[] memory employeeIds = mappingToArrays.getTwoKeyArray(
            movieId,
            departmentId
        );
        for (uint256 i = 0; i < totalEmployees; i++) {
            ...
            require(
                _balances[movieId][msg.sender][departmentId] >= employeeSalary,
                "Department:Insufficient Balance"
            );
            safeTransferFrom(
                msg.sender,
                employeeAddress,
    ...
    function removeEmployee(
        ...
        require(
            _balances[movieId][employeeAddress][departmentId] == 0,
        ...
        departments[movieId][departmentId].noOfEmployees--;
        mappingToArrays.removeFromTwoKeyMapping(
            movieId,
            departmentId,
            employeeId
        );
